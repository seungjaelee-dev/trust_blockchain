...
**Docker 준비:** [Docker 실행 안내](docs/DOCKER.md)에 환경 고정·개발용 소스 연결·오프라인 이미지 전달 명령을 정리했습니다.
Dockerfile과 패키지 버전 lock은 준비했으며, 현재 PC에 Docker가 없어 이미지 빌드/컨테이너 검증은 아직 미완료입니다.
...
1. Docker 실행 허용은 사용자 확인 완료. 채점 아키텍처, 이미지 반입 방식, 용량 제한·제출 경로 확인.
