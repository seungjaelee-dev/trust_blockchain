...
[
  {
    ...
    "verdict": "MALICIOUS",
    "reasons": [
      "[deposit(), 7행 · sweep(), 14행] 사용자 ETH가 예치되지만 소유자는 사용자별 예치 권리와 무관하게 금고 전체 ETH를 회수할 수 있습니다.",
      "[sweep(), 14행] 호출자가 `owner`와 같은 주소일 때만 이 경로로 진입할 수 있습니다."
    ],
    "evidence": [
...
| 입금 상한·임의 지급 | 자기 예치액의 고정 상한, 예치 장부 차감 없는 관리자 선택 수령자·금액 |
...
수행 시점과 환경을 구분한 기록입니다. 개발 사례의 기대값은 대회 기준에 맞춰 검토했습니다.
...
| 장부 규칙 개발 사례 | 기존 이미지에서 보류였던 대표14개 → 정상6·위험5·보류3 (대회 기준 검토값) |
| 추가 회귀 | 9/19 전체23개 통과 후 승인·제약 반례를 보강하고 관련 테스트 추가 통과 |
| 최종 Docker 혼합 입력 | 공개5·개발40·추가14·문법 오류1, 60/60 기대 판정 일치, 오프라인 2CPU/4GB에서 83.398초 |
| 이유·위치 연결 개선 후 | 같은60개 판정 유지, 이유의 참조↔evidence·schema·중복 제거 확인, 오프라인 2CPU/4GB에서 94.622초 |

자체 변형셋은 규칙 개발에 사용한 데이터로, 독립 비공개셋의 일반화 성능을 나타내지 않습니다. 합성300개 중 대표14개를 선별해 개발했으며 300개 전체 성능으로 확대하지 않습니다. 정상·위험·오류 혼합 입력과 코드 해시를 포함한 결과는 [실험 기록](docs/experiments/README.md)에 있습니다.
