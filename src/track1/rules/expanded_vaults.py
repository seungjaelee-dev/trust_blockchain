...
from ..reporting import contextual_result
...
from .expanded_tokens import initialized, owner_guard, guards, root, parameter_condition, result, is_uint
...
def analyze_contract(contract):
    ...
    constants = {v for v in contract.state_variables if v.is_constant and is_uint(values.get(v, ZERO))}
    if any(v != ledger and v not in owners and v not in locks and v not in constants for v in contract.state_variables):
    ...
    for function, path in runtime:
        ...
        if function.payable:
            def deposit_condition(g):
                if g == ("LESS", ZERO, VALUE):
                    return True
                if g[0] != "LESS_EQUAL" or g[1] != ("ADDITION", loc, VALUE):
                    return False
                cap = g[2] if is_uint(g[2]) else values.get(g[2][1]) if g[2][0] == "variable" and g[2][1] in constants else None
                return cap is not None and is_uint(cap) and cap[2] > 0
            if (all(deposit_condition(g) for g in conditions) and not path.calls and len(path.writes) == 1
        ...
        if (call["kind"] == "delegatecall" and not path.writes and ordinary_owner
            ...
            continue
        if (call["kind"] == "call" and not path.writes and ordinary_owner
                and is_parameter(call["destination"], function, "address")
                and call["value"] is not None and is_parameter(call["value"], function, "uint256")
                and len(call["arguments"]) == 1
                and (is_parameter(call["arguments"][0], function, "bytes")
                     or call["arguments"][0] == ("constant", "string", ""))):
            routes.append(call["node"])
            continue
    ...
    if routes and not deposits:
        raise Unsupported("임의 송금은 있으나 사용자 예치 자산의 귀속을 확인하지 못했습니다.")
    if not deposits:
        return finish("BENIGN", "지원 범위에 ETH 예치/수신 경로가 없고 예치 장부는 0에서 시작합니다. "
    ...
    if sweeps:
        return finish("MALICIOUS", "사용자 ETH가 예치되지만 소유자는 사용자별 예치 권리/차감과 무관하게 "
                      "금고 전체 ETH를 소유자 주소로 송금할 수 있습니다.", deposits + sweeps)
    if reentry:
        return finish("MALICIOUS", "자기 예치 잔액을 읽어 송금한 뒤 장부를 0으로 만듭니다. "
    ...
    return finish("BENIGN", "사용자 입금은 본인의 예치 장부에 더해지고, 모든 송금은 본인에게 지급되며 동일 금액을 본인 장부에 반영합니다. "
